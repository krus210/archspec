...
> type `$architecture-sync` or pick it from `/skills`. opencode: it loads via the
...
Every bash snippet below assumes `SKILL_DIR` is resolved first — this skill's bundled
assets (`scripts/`, `templates/`, `schema/`) live inside its own directory, so archspec
anchors on that directory instead of a Claude-only env var:
...
- Appends the managed archspec block to `AGENTS.md` (the canonical cross-tool file) and `CLAUDE.md`.
...
3. Run the sync entry point:
   ...
   ```bash
   bash "$SKILL_DIR/scripts/_py.sh" "$SKILL_DIR/scripts/sync.py" docs/SERVICE_MAP.yaml docs
   ```

4. Sanity-check the generated `.mmd`:

   ```bash
   bash "$SKILL_DIR/scripts/_py.sh" "$SKILL_DIR/scripts/validate_mermaid.py" docs/diagrams/*.mmd
   ```
...
1. Refuse if any of these files already exist *and* contain non-archspec content:
   ...
   - `AGENTS.md` / `CLAUDE.md` outside the `<!-- archspec:claude-block:start -->` markers.

2. Copy the seed:

   ```bash
   mkdir -p docs/adr docs/diagrams .servicemap
   cp "$SKILL_DIR/templates/SERVICE_MAP.template.yaml" docs/SERVICE_MAP.yaml
   cp "$SKILL_DIR/schema/servicemap.schema.json" .servicemap/schema.json
   ```

2a. **Top-level architecture spec ingest (opt-in).** Ask the user (use your host's structured multiple-choice tool if it has one — Claude Code `AskUserQuestion`, opencode `question` — otherwise present the options as a numbered list and **stop until the user replies**):
   ...
   - **Path provided** — read the file and keep its contents as **conversation context only** (do not parse). Then, before each subsequent confirm prompt, briefly recite what the spec says about *this* service (e.g. "top-level spec lists `matching-service → geo-service` for distance computation; confirm?"). Print one reminder to the user once at the start: "Top-level spec may be stale — if anything contradicts the actual code, answer based on code, not spec." The spec is a hint, never an override.
   - **`skip` / no path** — proceed with no cross-check; per-service init runs as before.

3. Replace placeholders. Ask the user (use your host's structured multiple-choice tool if it has one — Claude Code `AskUserQuestion`, opencode `question` — otherwise present the options as a numbered list and **stop until the user replies**; single batch) for: service name, team, language, repo URL, domain, primary owner handle (e.g. `@alice`), oncall handle. Replace `REPLACE-WITH-DATE` with today's ISO date (asked from the user — do **not** read the system clock).
...
3a0. **Service-level questionnaire** — fill `service.responsibilities` and `service.invariants` BEFORE running the auto-discovery scanner. These are the two most important free-form fields in the contract; leaving them as `TODO` makes the generated `ARCHITECTURE.md` near-useless.
   ...
   Ask the user (use your host's structured multiple-choice tool if it has one — Claude Code `AskUserQuestion`, opencode `question` — otherwise present the options as a numbered list and **stop until the user replies**) two free-text questions, both required:
   ...
   Also ask in the same batch (or a follow-up if your host's question tool caps at N questions): `bounded_aggregate` (default = service name), and the `consistency.model` (`eventual` or `strong`, default `eventual`).
...
3a. **Auto-discover from code** (Go services only — skip if `service.language != go`).
   ...
   ```bash
   bash "$SKILL_DIR/scripts/_py.sh" "$SKILL_DIR/scripts/scan_go.py" . > /tmp/archspec-scan.json
   ```
...
3b. **Interactive questionnaire** — confirm findings and fill in fields the scanner cannot supply.
   ...
   For each `endpoint` finding, ask the user (use your host's structured multiple-choice tool if it has one — Claude Code `AskUserQuestion`, opencode `question` — otherwise present the options as a numbered list and **stop until the user replies**; one batch per category for fewer round-trips):
   ...
   For each accepted endpoint, ask a follow-up batch (use your host's structured multiple-choice tool if it has one — Claude Code `AskUserQuestion`, opencode `question` — otherwise present the options as a numbered list and **stop until the user replies**; ask several questions together in one message):
   ...
   For each `downstream_sync` finding, ask the user (use your host's structured multiple-choice tool if it has one — Claude Code `AskUserQuestion`, opencode `question` — otherwise present the options as a numbered list and **stop until the user replies**) in one batch:
   ...
   After all categories, ask the user one trailing question (use your host's structured multiple-choice tool if it has one — Claude Code `AskUserQuestion`, opencode `question` — otherwise present the options as a numbered list and **stop until the user replies**) — and explicitly mention messaging because the scanner only knows Kafka and NATS:
...
3b-rev. **Discover upstream consumers** (who calls this service).
   ...
   Ask the user (use your host's structured multiple-choice tool if it has one — Claude Code `AskUserQuestion`, opencode `question` — otherwise present the options as a numbered list and **stop until the user replies**):
   ...
   - **Path provided** — run the reverse scanner:
     ...
     ```bash
     bash "$SKILL_DIR/scripts/_py.sh" \
       "$SKILL_DIR/scripts/scan_go.py" \
     ...
     **Apply the findings deterministically — do NOT rely on the LLM to remember to write each entry.** Use the merge script:
     ...
     ```bash
     bash "$SKILL_DIR/scripts/_py.sh" \
       "$SKILL_DIR/scripts/apply_upstream.py" \
       docs/SERVICE_MAP.yaml --reverse-scan-json /tmp/archspec-reverse.json
     ```

     Without `--write`, the script prints a unified diff. Show the diff to the user (use your host's structured multiple-choice tool if it has one — Claude Code `AskUserQuestion`, opencode `question` — otherwise present the options as a numbered list) and **do not proceed until the user answers**:
     ...
     For the «edit some» branch (rare): for each consumer, ask the user (use your host's structured multiple-choice tool if it has one — Claude Code `AskUserQuestion`, opencode `question` — otherwise present the options as a numbered list and **stop until the user replies**) to confirm/skip and edit the YAML by hand.

   - **`none` (not a monorepo, or repo not on disk)** — fall back to asking the user manually (use your host's structured multiple-choice tool if it has one — Claude Code `AskUserQuestion`, opencode `question` — otherwise present the options as a numbered list and **stop until the user replies**):
...
3b-agg. **Confirm detected aggregates** (Go scanner only). After the JSON report is read, iterate over `aggregates` and ask the user (use your host's structured multiple-choice tool if it has one — Claude Code `AskUserQuestion`, opencode `question` — otherwise present the options as a numbered list and **stop until the user replies**) one question per finding:
...
3c0. **Decide `consistency.write_path.pattern`** based on findings:
   ...
   In every non-trivial case, ask the user (use your host's structured multiple-choice tool if it has one — Claude Code `AskUserQuestion`, opencode `question` — otherwise present the options as a numbered list) and **do not proceed until the user answers**: "Choose the write-path pattern: `outbox` (recommended for services that publish events from a durable store), `direct` (synchronous writes, no event publishing — also correct for stateless gateways), `saga` (multi-step distributed transactions)." Pre-select the recommendation as the first option.

3c. **Write the confirmed findings to `docs/SERVICE_MAP.yaml`** — write it by editing the file. Build each YAML block from the confirmed answers; do not write rejected items. Schema fields and defaults:
...
5. Append the managed archspec block to the agent-instructions files. `AGENTS.md` is
   the canonical cross-tool file (Codex, opencode, Cursor, Gemini, Copilot all read it);
   `CLAUDE.md` is Claude Code's native file. Write the same managed block to both so the
   guidance is visible on every host — `AGENTS.md` first, since it is the universal one:

   ```bash
   for memf in AGENTS.md CLAUDE.md; do
     # skip CLAUDE.md when it is the same inode as AGENTS.md (a symlink / @import shim)
     if [ "$memf" = "CLAUDE.md" ] && [ AGENTS.md -ef CLAUDE.md ] 2>/dev/null; then continue; fi
     if ! grep -q "archspec:claude-block:start" "$memf" 2>/dev/null; then
       cat "$SKILL_DIR/templates/CLAUDE.archspec-block.md" >> "$memf"
     fi
   done
   ```

6. Install the pre-commit hook (only present under a full plugin/clone install; `hooks/`
   is not bundled inside this skill):

   ```bash
   HOOKS_INSTALL="${CLAUDE_PLUGIN_ROOT:-$SKILL_DIR/../..}/hooks/pre-commit/install_hooks.sh"
   if [ -f "$HOOKS_INSTALL" ]; then
     bash "$HOOKS_INSTALL"
   else
     echo "archspec: git hooks are not bundled in a skills-only install; install them from the archspec repo (hooks/pre-commit/install_hooks.sh) if you want the pre-commit gate."
   fi
   ```
...
2. Run the audit script:
   ...
   ```bash
   bash "$SKILL_DIR/scripts/_py.sh" \
     "$SKILL_DIR/scripts/check_architecture.py" \
